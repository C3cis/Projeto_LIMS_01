...
public class Persistencia_Produto_02 {
    ...
    public boolean inserirProduto(Model_Produto_02 produto) {
        ...
        try (CallableStatement cs = conexao.prepareCall(sql)) {
            ...
            cs.setDate(4, new java.sql.Date(produto.getA02_data_cadastro().getTime()));
            cs.setDate(5, new java.sql.Date(produto.getA02_data_chegada().getTime()));
    ...
    public boolean atualizarProduto(Model_Produto_02 produto) {
        ...
        try (CallableStatement cs = conexao.prepareCall(sql)) {
            ...
            cs.setDate(5, new java.sql.Date(produto.getA02_data_cadastro().getTime()));
            cs.setDate(6, new java.sql.Date(produto.getA02_data_chegada().getTime()));
    ...
    public Model_Produto_02 buscarProduto(int idProduto) {
        ...
        try (CallableStatement cs = conexao.prepareCall(sql)) {
            ...
            if (rs.next()) {
                ...
                produto.setA02_id_produto(rs.getInt("A02_ID_PRODUTO"));
                produto.setA02_nome_produto(rs.getString("A02_NOME_PRODUTO"));
                produto.setA02_descricao(rs.getString("A02_DESCRICAO"));
                produto.setA02_tipo(rs.getString("A02_TIPO"));
                produto.setA02_data_cadastro(rs.getDate("A02_DATA_CADASTRO"));
                produto.setA02_data_chegada(rs.getDate("A02_DATA_CHEGADA"));
                produto.setA02_valor_unitario(rs.getDouble("A02_VALOR_UNITARIO"));
                produto.setA02_id_projeto(rs.getInt("A02_ID_PROJETO"));
                produto.setA02_id_fornecedor(rs.getInt("A02_ID_FORNECEDOR"));
